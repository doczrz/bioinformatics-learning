...
当前可以切换整页中英文、复制代码、下载外部数据，并在框选正文或代码后点击“问 Codex”。本地 R、本地 Python 与 SSH/HPC 均仅保留为禁用接口，第一版不会运行代码。

The reader supports whole-page language switching, code copy, authoritative external dataset links, and an explicit “Ask Codex” flow after selecting lesson text or code. Local R, Local Python, and SSH/HPC remain disabled placeholders and execute nothing in version 1.
